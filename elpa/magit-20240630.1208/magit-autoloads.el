...
See variable `magit-define-global-key-bindings'." (when magit-define-global-key-bindings (let ((map (current-global-map))) (pcase-dolist ((\` ((\, key) \, def)) (cond ((eq magit-define-global-key-bindings (quote recommended)) (quote (("C-x g" . magit-status) ("C-c g" . magit-dispatch) ("C-c f" . magit-file-dispatch)))) ((quote (("C-x g" . magit-status) ("C-x M-g" . magit-dispatch) ("C-c M-g" . magit-file-dispatch)))))) (when (or force (not (or (lookup-key map (kbd key)) (where-is-internal def (make-sparse-keymap) t)))) (define-key map (kbd key) def))))))
...
(if (fboundp 'register-definition-prefixes) (register-definition-prefixes "magit-files" '("magit-" "lsp")))
